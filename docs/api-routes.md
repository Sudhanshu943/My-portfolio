...
|--------|----------|------|-------------|
| `POST` | `/api/admin/login` | None | Authenticate with `ADMIN_PASSWORD`, sets `admin_session` httpOnly cookie |
| `POST` | `/api/admin/logout` | Admin | Clears `admin_session` cookie |
| `GET` | `/api/admin/check` | Admin | Returns current admin session status |
